...
import { lessonModerators, lessonPayload } from "~/services/interfaces";
import { decryptToken } from "~/services/tokenManager";

function EditLesson({ setEditLesson, id }: lessonModerators) {
  ...
  const handleCreateLesson = async () => {
    ...
    let cookieValue = Cookies.get(uuidName);
     if (cookieValue) {
      ...
      const response = (
        await api.post("teacher-api/update-lesson",lesson, {
          headers: {
    ...
    <div className="p-4 w-full md:w-[80%] md:mx-[10%] h-screen bg-white overflow-y-scroll">
      <div className="flex items-center justify-between">
        ...
        <IoClose
          ...
          onClick={() => setEditLesson && setEditLesson(false)}
        />
      </div>
